...
func (c Command) String() string {
	...
	case "base":
		fmt.Fprintf(&sb, "BASE %s", quote(c.Args))
	case "key":
		fmt.Fprintf(&sb, "KEY %s", quote(c.Args))
	case "model":
		fmt.Fprintf(&sb, "MODEL %s", quote(c.Args))
	default:
...
func isValidCommand(cmd string) bool {
	...
	case "llm", "license", "template", "system", "adapter", "parameter", "message", "base", "model", "key":
		return true
	default:
		return false
	}
